...
class MakeFoldsDisjoint:
    ...
    def __init__(self,root_dir='',norm_root_dir=''):
        ...
        self.train_sets = self.make_foldsets_disjoint(self.train_sets)
        self.val_sets = self.make_foldsets_disjoint(self.val_sets)
        self.test_sets = self.make_foldsets_disjoint(self.test_sets)
    ...
    def write(self, out_dir, norm_out_dir):
        """ Write out train, val and test folds"""
...
#        self.train_folds,self.val_folds,self.test_folds,self.norm_train_folds,self.norm_val_folds,self.norm_test_folds=self.convert_fold_types_to_tokens()
    ...
    def load_folds(self, root_dir='',norm_root_dir=''):
        ...
        for i in tqdm(range(4), desc="Loading train data"):
            ...
            self.train_sets.append(set(train))
            #self.train_tups+=list(zip(train,norm_train))
            #self.train_tups = [(x,y) for (x,y) in zip(train,norm_train) if len(y)>3 and abs(len(y)-len(x))<2]
            self.train_tups = [(x,y) for (x,y) in zip(train,norm_train) if len(y)>1]
        ...
        for i in tqdm(range(4), desc="Loading val data"):
            ...
            #self.val_tups = [(x,y) for (x,y) in zip(val,norm_val) if len(y)>3 and abs(len(y)-len(x))<2]
            self.val_tups = [(x,y) for (x,y) in zip(val,norm_val) if len(y)>1]
...
#            self.test_tups+=list(zip(test,norm_test))
            #self.test_tups = [(x,y) for (x,y) in zip(test,norm_test) if len(y)>3 and abs(len(y)-len(x))<2]
            self.test_tups = [(x,y) for (x,y) in zip(test,norm_test) if len(y)>1]
    ...
    def check_fold_dependencies(self, norm_folds):
        for i in range(len(norm_folds)):
        ...
        for typ in tqdm(types,desc=f"Converting {norm_str}fold{fold_num} types to tokens"):
            for tup in tups:
                if typ==tup[1]:
                    ...
                    #print(tup[0],typ)
                    typ_found_flag==True
            if typ_found_flag==False:
                token_fold.append(typ)
                norm_token_fold.append(typ)
...
if __name__ == "__main__":
    make_dis = MakeFoldsDisjoint(root_dir="folds_enc_dec/foldset",norm_root_dir="folds_enc_dec/norm_foldset")
    make_dis.write(out_dir="folds/foldset",norm_out_dir="folds/norm_foldset")
